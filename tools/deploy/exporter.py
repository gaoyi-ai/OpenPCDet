...
class DemoKittiDataset(DatasetTemplate):
...
class DemoNuscenesDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, ext='.bin'):
        """
        Args:
            root_path:
...
def export_centerpoint_nuscenes(): 
    args, cfg = parse_config()
    logger = common_utils.create_logger()
    logger.info('------ Convert OpenPCDet model for TensorRT ------')
    demo_dataset = DemoNuscenesDataset(
        dataset_cfg=cfg.DATA_CONFIG, class_names=cfg.CLASS_NAMES, training=False,
        root_path=Path(args.data_path), ext=args.ext, logger=logger
    )
    ...
    with torch.no_grad():
        ...
        dummy_input = dict()
        dummy_input['points'] = data_dict['points']
        dummy_input['batch_size'] = 1
        ...
        torch.onnx.export(model,  # model being run
                          ...
                          do_constant_folding=True,  # whether to execute constant folding for optimization
                          keep_initializers_as_inputs=True,
                          input_names=['points'],  # the model's input names
                          output_names=['cls_preds', 'box_preds', 'dir_cls_preds'],  # the model's output names
                        #   dynamic_axes={'points' : {1 : 'batch_size'},    # variable length axes 
                        #                 },
                          )
...
def parse_config():
    ...
    parser.add_argument('--cfg_file', type=str, default='/home/robot/OpenPCDet/tools/cfgs/nuscenes_models/cbgs_dyn_pp_centerpoint_mini.yaml',
                        help='specify the config for demo')
    parser.add_argument('--data_path', type=str, default='/home/robot/OpenPCDet/data/nuscenes/v1.0-mini/samples/LIDAR_TOP/n008-2018-08-01-15-16-36-0400__LIDAR_TOP__1533151603547590.pcd.bin',
                        help='specify the point cloud data file or directory')
    parser.add_argument('--ckpt', type=str, default='/home/robot/OpenPCDet/checkpoints/cbgs_pp_centerpoint_nds6070.pth', help='specify the pretrained model')
...
if __name__ == '__main__':
    # export_pointpillars_kitti()
    export_centerpoint_nuscenes()
    # onnx_check()
